...
export const materialDescription: INodeProperties[] = [
	{
		...
		options: [
			{ name: '上传临时素材', value: 'uploadTemp', action: '上传临时素材', description: '上传临时素材（图片、语音、视频、文件），media_id仅三天内有效' },
			{ name: '上传图片', value: 'uploadImage', action: '上传图片', description: '上传图片素材获取URL（永久有效），仅用于图文消息等' },
			{ name: '获取临时素材', value: 'getTemp', action: '获取临时素材', description: '下载已上传的临时素材文件' },
			{ name: '获取高清语音素材', value: 'getHighQualityVoice', action: '获取高清语音素材', description: '下载高清语音素材文件（speex格式，16K采样率）' },
			{ name: '异步上传临时素材', value: 'uploadTempAsync', action: '异步上传临时素材', description: '通过CDN URL异步上传大文件（最大200MB），返回jobid' },
			{ name: '查询异步任务结果', value: 'getUploadByUrlResult', action: '查询异步任务结果', description: '通过jobid查询异步上传任务的结果' },
		],
		default: 'uploadTemp',
	},
